...
#include "bio_driver_server.h"

void labstor::BIODriver::Server::ProcessRequest(labstor::ipc::queue_pair *qp, labstor::ipc::request *request, labstor::credentials *creds) {
    AUTO_TRACE(request->op_, request->req_id_)
    switch (static_cast<Ops>(request->op_)) {
        case Ops::kWrite:
        case Ops::kRead: {
            IOStart(qp, reinterpret_cast<labstor_bio_driver_request*>(request), creds);
            break;
        }
        case Ops::kIOComplete: {
            IOComplete(qp, reinterpret_cast<labstor_bio_driver_poll_request*>(request));
            break;
        }
    }
}

void labstor::BIODriver::Server::IOStart(labstor::ipc::queue_pair *qp, labstor_bio_driver_request *client_rq, labstor::credentials *creds) {
    AUTO_TRACE(client_rq->dev_id_);
    labstor_bio_driver_poll_request *poll_rq;
    labstor_bio_driver_request *kern_rq;
    ...
    poll_rq = ipc_manager_->AllocRequest<labstor_bio_driver_poll_request>(private_qp);
    poll_rq->Init(qp, client_rq, qtok);
    private_qp->Enqueue<labstor_bio_driver_poll_request>(poll_rq, qtok);
}

void labstor::BIODriver::Server::IOComplete(labstor::ipc::queue_pair *private_qp, labstor_bio_driver_poll_request *poll_rq) {
    ...
    if(!kern_qp->IsComplete<labstor_bio_driver_request>(poll_rq->poll_qtok_, kern_rq)) {
        private_qp->Enqueue<labstor_bio_driver_poll_request>(poll_rq, qtok);
        return;
    }
    ...
    qp->Complete<labstor_bio_driver_request>(poll_rq->reply_qtok_, client_rq);

    //Free completed requests
    TRACEPOINT("Complete");
    ipc_manager_->FreeRequest<labstor_bio_driver_request>(kern_qp, kern_rq);
    ipc_manager_->FreeRequest<labstor_bio_driver_poll_request>(private_qp, poll_rq);
}

LABSTOR_MODULE_CONSTRUCT(labstor::BIODriver::Server);