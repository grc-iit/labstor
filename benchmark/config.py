...
class LabStorConfigurator:
    ...
    def GetBothServerAffinities(self):
        ...
        cpu_set = {False for cpu in range(num_cpus)}
    ...
    def GetApplicationAffinity(self):
        num_cpus = multiprocessing.cpu_count()
        cpu_set = {True for cpu in range(num_cpus)}
        for worker_conf in self.conf["work_orchestrator"]["kernel_workers"]:
    ...
    def SetFractionalWorkers(self, n_server_workers, ncpu, kern_frac):
        self.ResetWorkers()
