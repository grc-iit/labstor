...
static inline uint32_t labstor_ring_buffer_uint32_t_GetSize_global(uint32_t max_depth) {
    return sizeof(struct labstor_ring_buffer_uint32_t_header) + sizeof(uint32_t)*max_depth;
}
...
static inline bool labstor_ring_buffer_uint32_t_Init(struct labstor_ring_buffer_uint32_t *rbuf, void *region, uint32_t region_size, uint32_t max_depth) {
    ...
    rbuf->queue_ = (uint32_t*)(rbuf->header_ + 1);
    rbuf->header_->enqueued_ = 0;
    rbuf->header_->dequeued_ = 0;
    if(max_depth > 0) {
        min_region_size = labstor_ring_buffer_uint32_t_GetSize_global(max_depth);
        if(min_region_size > region_size) {
            //pr_warn("labstor_ring_buffer_uint32_t_Init: depth of %u requires at least %u bytes", max_depth, min_region_size);
            return false;
        }
        rbuf->header_->max_depth_ = max_depth;
    } else {
        rbuf->header_->max_depth_ = (region_size - sizeof(struct labstor_ring_buffer_uint32_t_header)) / sizeof(uint32_t);
    }
    return true;
}
...
static inline bool labstor_ring_buffer_uint32_t_Enqueue(struct labstor_ring_buffer_uint32_t *rbuf, uint32_t data, uint32_t *req_id) {
    ...
    do {
        ...
        if(labstor_ring_buffer_uint32_t_GetDepth(rbuf) > rbuf->header_->max_depth_) { return false; }
    }
...
struct ring_buffer_uint32_t_header {
    uint64_t enqueued_, dequeued_;
    uint32_t max_depth_;
};

class ring_buffer_uint32_t : private labstor_ring_buffer_uint32_t, public shmem_type {
public:
    ...
    inline void* GetRegion() { return labstor_ring_buffer_uint32_t_GetRegion(this); }
