...
static inline bool labstor_ring_buffer_uint32_t_WaitForClear(struct labstor_ring_buffer_uint32_t *rbuf, uint32_t bit) {
...
static inline bool labstor_ring_buffer_uint32_t_WaitForSet(struct labstor_ring_buffer_uint32_t *rbuf, uint32_t bit) {
    int max_retries = 10000, i = 0;
    while(!labstor_bitmap_IsSet(rbuf->bitmap_, bit) && i < max_retries) { ++i; LABSTOR_YIELD(); }
    return i < max_retries;
}
...
static inline bool labstor_ring_buffer_uint32_t_Dequeue(struct labstor_ring_buffer_uint32_t *rbuf, uint32_t *data) {
    ...
    if(labstor_ring_buffer_uint32_t_WaitForSet(rbuf, entry)) {
