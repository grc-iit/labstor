...
static inline bool labstor_ring_buffer_qp_ptr_Init(struct labstor_ring_buffer_qp_ptr *rbuf, void *region, uint32_t region_size, uint32_t max_depth) {
    ...
    if(max_depth > 0) {
        ...
        header_size = sizeof(struct labstor_ring_buffer_qp_ptr_header) + labstor_bitmap_GetSize(max_depth);
        rbuf->header_->max_depth_ = (region_size - header_size) / sizeof(struct labstor_queue_pair_ptr);
    }
