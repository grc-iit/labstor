...
static inline bool labstor_ring_buffer_{T_NAME}_WaitForClear(struct labstor_ring_buffer_{T_NAME} *rbuf, uint32_t bit) {
...
static inline bool labstor_ring_buffer_{T_NAME}_WaitForSet(struct labstor_ring_buffer_{T_NAME} *rbuf, uint32_t bit) {
    int max_retries = 10000, i = 0;
    while(!labstor_bitmap_IsSet(rbuf->bitmap_, bit) && i < max_retries) { ++i; LABSTOR_YIELD(); }
    return i < max_retries;
}
...
static inline bool labstor_ring_buffer_{T_NAME}_Dequeue(struct labstor_ring_buffer_{T_NAME} *rbuf, {T} *data) {
    ...
    if(labstor_ring_buffer_{T_NAME}_WaitForSet(rbuf, entry)) {
