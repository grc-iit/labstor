...
void labstor::WorkOrchestrator::CreateWorkers() {
    auto labstor_context_ = scs::Singleton<labstor::LabStorServerContext>::GetInstance();
    auto labstor_kernel_context_ = scs::Singleton<labstor::LabStorKernelClientContext>::GetInstance();
    const auto &config = labstor_context_->config_["work_orchestrator"];
    ...
    //pthread_create(&process_wreaper_, );

    //Server worker threads
    cpu_set_t cpus[n_cpu_];
    CPU_ZERO(cpus);
    nworkers = config["server_workers"].size();
    worker_pool_.emplace(labstor_context_->pid_, nworkers);
    auto &server_workers_ = worker_pool_[labstor_context_->pid_];
    ...
    shmem.GrantPidShmem(labstor_context_->pid_, region_id);
    ...
    auto &kernel_workers_ = worker_pool_[labstor_context_->pid_];
